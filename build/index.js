...
class AgentProxyServer {
            ...
            try {
                ...
                switch (name) {
                    case "agentproxy_fetch": {
                        if (!PROXY_API_KEY)
                            return this.missingKeyError("PROXY_API_KEY", "fetch/session tools (proxy network)");
                        result = await agentproxyFetch(validateFetchParams(raw), PROXY_API_KEY);
                        break;
                    }
                    case "agentproxy_render": {
                        if (!NOVADA_API_KEY)
                            return this.missingKeyError("NOVADA_API_KEY", "render tool (Novada Browser API)");
                        result = await agentproxyRender(validateRenderParams(raw), NOVADA_API_KEY);
                        break;
                    }
                    case "agentproxy_search": {
                        if (!NOVADA_API_KEY)
                            return this.missingKeyError("NOVADA_API_KEY", "search tool (Novada Scraper API)");
                ...
                let message = rawMsg;
                if (NOVADA_API_KEY)
                    message = message.replace(NOVADA_API_KEY, "***");
                if (PROXY_API_KEY && PROXY_API_KEY !== NOVADA_API_KEY)
                    message = message.replace(PROXY_API_KEY, "***");
                return {
    ...
    missingKeyError(envVar, forWhat) {
        return {
            content: [{
                    type: "text",
