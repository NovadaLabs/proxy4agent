...
function makeFetchResponse(url, html) {
...
describe("validateMapParams", () => {
    ...
    it("rejects non-http url", () => {
        expect(() => validateMapParams({ url: "ftp://example.com" })).toThrow("url must start with http://");
    });
    it("rejects limit below 10", () => {
        expect(() => validateMapParams({ url: "https://example.com", limit: 5 })).toThrow("limit must be between 10 and 200");
    });
    it("rejects limit above 200", () => {
        expect(() => validateMapParams({ url: "https://example.com", limit: 300 })).toThrow("limit must be between 10 and 200");
    });
    it("rejects country with hyphens", () => {
        expect(() => validateMapParams({ url: "https://example.com", country: "us-east" })).toThrow("country must be a 2-letter ISO code");
    });
    it("rejects timeout out of range", () => {
        expect(() => validateMapParams({ url: "https://example.com", timeout: 0 })).toThrow("timeout must be between 1 and 120");
        expect(() => validateMapParams({ url: "https://example.com", timeout: 121 })).toThrow("timeout must be between 1 and 120");
    });
...
describe("novadaProxyMap", () => {
    ...
    it("extracts internal links from HTML", async () => {
      ...
      <a href="/blog">Blog</a>
        ...
        novadaProxyFetchSpy.mockResolvedValueOnce(makeFetchResponse("https://example.com", html));
        const raw = await novadaProxyMap({ url: "https://example.com" }, mockAdapter, mockCreds);
        const result = JSON.parse(raw);
        expect(result.ok).toBe(true);
        expect(result.tool).toBe("novada_proxy_map");
        expect(result.data.domain).toBe("example.com");
        expect(result.data.internal_url_count).toBe(3);
        const urls = result.data.internal_urls;
    ...
    it("includes external URLs when include_external is true", async () => {
      ...
      <a href="https://other.com/page">External</a>
        ...
        novadaProxyFetchSpy.mockResolvedValueOnce(makeFetchResponse("https://example.com", html));
        const raw = await novadaProxyMap({ url: "https://example.com", include_external: true }, mockAdapter, mockCreds);
        const result = JSON.parse(raw);
        expect(result.data.internal_url_count).toBe(1);
        expect(result.data.external_url_count).toBe(1);
        const external = result.data.external_urls;
    ...
    it("caps results at the limit parameter", async () => {
        ...
        novadaProxyFetchSpy.mockResolvedValueOnce(makeFetchResponse("https://example.com", html));
        const raw = await novadaProxyMap({ url: "https://example.com", limit: 10 }, mockAdapter, mockCreds);
        const result = JSON.parse(raw);
        const urls = result.data.internal_urls;
    ...
    it("resolves relative URLs to absolute", async () => {
      ...
      <a href="subpage">Also relative</a>
        ...
        novadaProxyFetchSpy.mockResolvedValueOnce(makeFetchResponse("https://example.com/base/", html));
        const raw = await novadaProxyMap({ url: "https://example.com/base/" }, mockAdapter, mockCreds);
        const result = JSON.parse(raw);
        const urls = result.data.internal_urls;
    ...
    it("throws on invalid URL", async () => {
        await expect(novadaProxyMap({ url: "not-a-url" }, mockAdapter, mockCreds)).rejects.toThrow("Invalid URL");
    });
    it("returns correct response format", async () => {
        novadaProxyFetchSpy.mockResolvedValueOnce(makeFetchResponse("https://example.com", "<html><body></body></html>"));
        const raw = await novadaProxyMap({ url: "https://example.com" }, mockAdapter, mockCreds);
        const result = JSON.parse(raw);
    ...
    it("includes sitemap_hint when sitemap URL not found on page", async () => {
        novadaProxyFetchSpy.mockResolvedValueOnce(makeFetchResponse("https://example.com", "<html><body>No links</body></html>"));
        const raw = await novadaProxyMap({ url: "https://example.com" }, mockAdapter, mockCreds);
        const result = JSON.parse(raw);
        expect(result.data.sitemap_hint).toContain("sitemap.xml");
    });
    it("handles page with no links", async () => {
        novadaProxyFetchSpy.mockResolvedValueOnce(makeFetchResponse("https://example.com", "<html><body>No links here</body></html>"));
        const raw = await novadaProxyMap({ url: "https://example.com" }, mockAdapter, mockCreds);
        const result = JSON.parse(raw);
