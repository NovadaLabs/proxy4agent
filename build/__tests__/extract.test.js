...
describe("validateExtractParams — schema validation", () => {
    ...
    it("rejects schema keys with special chars", () => {
        expect(() => validateExtractParams({ url: baseUrl, schema: { "bad-key": "description" } })).toThrow("schema keys must be alphanumeric/underscore");
    });
    it("rejects schema keys that start with a number", () => {
        expect(() => validateExtractParams({ url: baseUrl, schema: { "1field": "description" } })).toThrow("schema keys must be alphanumeric/underscore");
    });
    it("rejects schema keys over 50 chars", () => {
        const longKey = "a".repeat(51);
        expect(() => validateExtractParams({ url: baseUrl, schema: { [longKey]: "description" } })).toThrow("schema keys must be alphanumeric/underscore");
    });
    it("rejects whitespace-only schema values", () => {
        expect(() => validateExtractParams({ url: baseUrl, schema: { price: "   " } })).toThrow("schema values must be non-empty strings");
    });
    ...
    it("rejects schema values over 200 chars", () => {
        ...
        expect(() => validateExtractParams({ url: baseUrl, schema: { price: longVal } })).toThrow("schema value descriptions must be 200 characters or less");
    });
    it("rejects schema with 0 fields", () => {
        expect(() => validateExtractParams({ url: baseUrl, schema: {} })).toThrow("schema must have at least 1 field");
    });
    it("rejects schema with more than 20 fields", () => {
        const bigSchema = {};
        for (let i = 1; i <= 21; i++)
            bigSchema[`field${i}`] = "description";
        expect(() => validateExtractParams({ url: baseUrl, schema: bigSchema })).toThrow("schema must have at most 20 fields");
    });
    it("still requires fields when schema is absent", () => {
        expect(() => validateExtractParams({ url: baseUrl })).toThrow("fields is required");
    });
});
...
describe("novadaProxyExtract — schema mode integration", () => {
    ...
    const testHtml = "<html><body><h1>Widget Pro</h1><p>Price: $49.99</p><p>Warranty: 1 year.</p></body></html>";
    const mockAdapter = {
    ...
    const mockCredentials = { user: "u", pass: "p" };
    async function setupFetchMock(html) {
        const { novadaProxyFetch } = await import("../tools/fetch.js");
        vi.mocked(novadaProxyFetch).mockResolvedValue(JSON.stringify({ ok: true, tool: "novada_proxy_fetch", data: { content: html, status_code: 200 }, meta: { latency_ms: 10 } }));
    }
    ...
    it("content is cleaned markdown (not raw HTML)", async () => {
        ...
        expect(data.content).not.toMatch(/<html>/i);
        expect(data.content).not.toMatch(/<body>/i);
        // Should contain the text content
        expect(data.content).toContain("Widget Pro");
    });
    it("content is truncated with unicodeSafeTruncate (max 50000 chars)", async () => {
        ...
        const result = JSON.parse(resultStr);
        const data = result.data;
        expect(data.content.length).toBeLessThanOrEqual(50000);
        expect(data.content_length).toBeLessThanOrEqual(50000);
    });
    it("extraction_prompt contains schema fields but NOT the page content", async () => {
        ...
        const result = JSON.parse(resultStr);
        const data = result.data;
        const prompt = data.extraction_prompt;
