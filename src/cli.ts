...
async function handleResearch(args: string[]): Promise<void> {
  ...
  if (values.help) { printSubcommandHelp("research"); return; }

  const query = positionals.join(" ");
  if (!query) invalidArgs("research", "Missing required argument: <query>");
...
async function handleExtract(args: string[]): Promise<void> {
  ...
  if (!values.fields && !values.schema) invalidArgs("extract", "Missing required option: --fields title,price,description or --schema '{\"field\":\"description\"}'");
  ...
  if (values.schema) {
    try {
      schema = JSON.parse(values.schema as string) as Record<string, string>;
    } catch {
  ...
  const fields = values.fields
    ? (values.fields as string).split(",").map((f: string) => f.trim()).filter(Boolean)
    : [];
...
async function main(): Promise<void> {
  ...
  try {
    switch (subcommand) {
      ...
      case "research": await handleResearch(restArgs); break;
