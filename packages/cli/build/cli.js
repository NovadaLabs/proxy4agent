...
function writeError(err, exitCode = 1) {
...
function missingBrowserWsError() {
    const errResp = {
        ok: false,
        error: {
            code: "PROVIDER_NOT_CONFIGURED",
...
function invalidArgs(subcommand, message) {
    const errResp = {
        ok: false,
        error: {
            code: "INVALID_INPUT",
...
function prettyPrint(result) {
    try {
        ...
        if (tool)
            process.stdout.write(`\x1b[1;36m${tool}\x1b[0m\n`);
        // Status
        if (parsed.ok) {
            ...
            if (content) {
                ...
                for (const [key, val] of Object.entries(data)) {
                    if (Array.isArray(val)) {
                        ...
                        if (val.length > 20)
                            process.stdout.write(`  ... and ${val.length - 20} more\n`);
                    }
...
async function handleFetch(args) {
    ...
    if (values.help) {
        printSubcommandHelp("fetch");
        return;
    }
    const url = positionals[0];
    if (!url)
        invalidArgs("fetch", "Missing required argument: <url>");
    if (!proxyContext)
        missingProxyError();
...
async function handleBatch(args) {
    ...
    if (values.help) {
        printSubcommandHelp("batch");
        return;
    }
    if (positionals.length < 2)
        invalidArgs("batch", "batch requires at least 2 URLs");
    if (!proxyContext)
        missingProxyError();
...
async function handleSearch(args) {
    ...
    if (!query)
        invalidArgs("search", "Missing required argument: <query>");
    if (!NOVADA_API_KEY)
        missingApiKeyError();
...
async function handleMap(args) {
    ...
    if (values.help) {
        printSubcommandHelp("map");
        return;
    }
    const url = positionals[0];
    if (!url)
        invalidArgs("map", "Missing required argument: <url>");
    if (!proxyContext)
        missingProxyError();
...
async function handleRender(args) {
    ...
    if (values.help) {
        printSubcommandHelp("render");
        return;
    }
    const url = positionals[0];
    if (!url)
        invalidArgs("render", "Missing required argument: <url>");
    if (!NOVADA_BROWSER_WS)
...
async function handleSession(args) {
    ...
    if (!sessionId)
        invalidArgs("session", "Missing required argument: <session_id>");
    if (!url)
        invalidArgs("session", "Missing required argument: <url>");
    if (!proxyContext)
        missingProxyError();
