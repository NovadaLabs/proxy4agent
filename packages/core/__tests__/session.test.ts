...
import { novadaProxySession, validateSessionParams } from "../src/tools/session.js";
...
function makeFetchResponse(url: string, content: string): string {
...
describe("validateSessionParams", () => {
  ...
  it("rejects session_id with hyphens", () => {
    expect(() =>
      validateSessionParams({ session_id: "my-session", url: "https://example.com" })
  ...
  it("rejects session_id over 64 chars", () => {
    ...
    expect(() =>
      validateSessionParams({ session_id: longId, url: "https://example.com" })
    ).toThrow("session_id is required");
  });
  ...
  it("rejects non-http url", () => {
    expect(() =>
      validateSessionParams({ session_id: "abc", url: "ftp://example.com" })
    ).toThrow("url must start with http://");
  });

  it("rejects country with hyphens", () => {
    expect(() =>
      validateSessionParams({ session_id: "abc", url: "https://example.com", country: "us-east" })
    ).toThrow("country must be a 2-letter ISO code");
  });

  it("rejects city with special characters", () => {
    expect(() =>
      validateSessionParams({ session_id: "abc", url: "https://example.com", city: "new-york" })
    ).toThrow("city must contain only letters");
  });

  it("rejects invalid format", () => {
    expect(() =>
      validateSessionParams({ session_id: "abc", url: "https://example.com", format: "html" })
    ).toThrow("format must be 'raw' or 'markdown'");
  });

  it("rejects timeout out of range", () => {
    expect(() =>
      validateSessionParams({ session_id: "abc", url: "https://example.com", timeout: 0 })
...
describe("novadaProxySession", () => {
  ...
  it("returns content with session_id in meta", async () => {
    novadaProxyFetchSpy.mockResolvedValueOnce(
      makeFetchResponse("https://example.com", "# Hello World")
    );

    const raw = await novadaProxySession(
      { session_id: "mysession", url: "https://example.com" },
      mockAdapter,
      mockCreds
    );
  ...
  it("passes session_id to novadaProxyFetch", async () => {
    novadaProxyFetchSpy.mockResolvedValueOnce(
      makeFetchResponse("https://example.com", "content")
    );

    await novadaProxySession(
      { session_id: "test123", url: "https://example.com", country: "US" },
      mockAdapter,
      mockCreds
    );

    expect(novadaProxyFetchSpy).toHaveBeenCalledWith(
  ...
  it("verifies sticky session when verify_sticky=true", async () => {
    ...
    const raw = await novadaProxySession(
      { session_id: "sticky1", url: "https://example.com", verify_sticky: true },
      mockAdapter,
      mockCreds
    );
    const result = JSON.parse(raw) as ProxySuccessResponse;
  ...
  it("reports session_verified=false when IPs differ", async () => {
    ...
    const raw = await novadaProxySession(
      { session_id: "sticky2", url: "https://example.com", verify_sticky: true },
      mockAdapter,
      mockCreds
    );
    const result = JSON.parse(raw) as ProxySuccessResponse;
  ...
  it("reports session_verified=false when verification calls fail", async () => {
    ...
    const raw = await novadaProxySession(
      { session_id: "sticky3", url: "https://example.com", verify_sticky: true },
      mockAdapter,
      mockCreds
    );
    const result = JSON.parse(raw) as ProxySuccessResponse;
  ...
  it("skips verification when adapter lacks sticky capability", async () => {
    const noStickyAdapter: ProxyAdapter = {
      ...mockAdapter,
    ...
    const raw = await novadaProxySession(
      { session_id: "s1", url: "https://example.com", verify_sticky: true },
      noStickyAdapter,
      mockCreds
    );
    const result = JSON.parse(raw) as ProxySuccessResponse;
  ...
  it("uses format from params or defaults to markdown", async () => {
    novadaProxyFetchSpy.mockResolvedValueOnce(
      makeFetchResponse("https://example.com", "<html>raw</html>")
    );

    await novadaProxySession(
      { session_id: "s1", url: "https://example.com", format: "raw" },
      mockAdapter,
      mockCreds
    );

    expect(novadaProxyFetchSpy).toHaveBeenCalledWith(
      expect.objectContaining({ format: "raw" }),
      mockAdapter,
      mockCreds
    );
  });
