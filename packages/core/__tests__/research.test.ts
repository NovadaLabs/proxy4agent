...
import { validateResearchParams, novadaProxyResearch } from "../src/tools/research.js";
...
function makeSearchResponse(results: Array<{ title: string; url: string; snippet: string }>): string {
...
function makeBatchResponse(
  results: Array<{ url: string; ok: boolean; content?: string; error?: { code: string; message: string } }>
): string {
  return JSON.stringify({
    ok: true,
...
describe("validateResearchParams", () => {
  ...
  it("rejects invalid depth", () => {
    expect(() => validateResearchParams({ query: "test", depth: "ultra" })).toThrow(
      "depth must be 'quick', 'standard', or 'deep'"
    );
  });

  it("rejects country with hyphens", () => {
    expect(() => validateResearchParams({ query: "test", country: "us-injected" })).toThrow(
  ...
  it("rejects timeout out of range", () => {
    expect(() => validateResearchParams({ query: "test", timeout: 0 })).toThrow(
      "timeout must be between 1 and 120"
    );
    expect(() => validateResearchParams({ query: "test", timeout: 121 })).toThrow(
      "timeout must be between 1 and 120"
    );
  });
...
describe("novadaProxyResearch", () => {
  ...
  it("returns valid JSON with correct tool name", async () => {
    novadaProxySearchSpy.mockResolvedValue(
      makeSearchResponse([
        { title: "Result 1", url: "https://example.com/1", snippet: "Snippet 1" },
        { title: "Result 2", url: "https://example.com/2", snippet: "Snippet 2" },
      ])
    );
    novadaProxyBatchFetchSpy.mockResolvedValue(
      makeBatchResponse([
    ...
    const raw = await novadaProxyResearch(
      { query: "test query" },
      mockAdapter,
      mockCreds,
      mockApiKey
  ...
  it("uses quick depth (3 sources)", async () => {
    ...
    expect(novadaProxySearchSpy).toHaveBeenCalledWith(
  ...
  it("uses standard depth (5 sources)", async () => {
    ...
    await novadaProxyResearch(
      { query: "test", depth: "standard" },
      mockAdapter,
      mockCreds,
      mockApiKey
    );
  ...
  it("handles search failure by propagating error", async () => {
    ...
    ).rejects.toThrow("Search API HTTP 500");
  });
