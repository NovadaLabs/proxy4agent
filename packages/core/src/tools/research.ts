...
export async function novadaProxyResearch(
  ...
  // Step 1: Search
  const searchResult = await novadaProxySearch(
    { query, num: numSources, country },
    novadaApiKey
  );
  const searchParsed = JSON.parse(searchResult) as ProxySuccessResponse;
  const searchResults = (searchParsed.data.results as Array<{ title: string; url: string; snippet: string }>) || [];
  ...
  let batchResults: Array<{ url: string; ok: boolean; content?: string; error?: { code: string; message: string } }> = [];

  if (urls.length === 1) {
    // batch_fetch requires minimum 2 URLs — fetch single URL directly
    try {
      const fetchResult = await novadaProxyFetch(
        { url: urls[0], format: "markdown", country, timeout },
        adapter,
        credentials
      );
      const parsed = JSON.parse(fetchResult) as ProxySuccessResponse;
      batchResults = [{ url: urls[0], ok: true, content: parsed.data.content as string }];
    } catch {
      batchResults = [{ url: urls[0], ok: false, error: { code: "FETCH_FAILED", message: "Failed to fetch" } }];
    }
  } else {
    const batchResult = await novadaProxyBatchFetch(
      { urls, format: "markdown", country, timeout, concurrency: 3 },
      adapter,
      credentials
    );
    const batchParsed = JSON.parse(batchResult) as ProxySuccessResponse;
    batchResults = (batchParsed.data.results as typeof batchResults) || [];
  }
...
export function validateResearchParams(raw: Record<string, unknown>): ResearchParams {
  ...
  if (raw.depth !== undefined && !["quick", "standard", "deep"].includes(raw.depth as string)) {
  ...
  return {
    ...
    depth: (raw.depth as "quick" | "standard" | "deep") || "standard",
